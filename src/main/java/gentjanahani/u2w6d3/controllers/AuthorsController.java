...
public class AuthorsController {
    ...
    @GetMapping
    public Page<Authors> getAuthors(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "5") int size) {
        return this.authorsService.findAuthors(page, size);
    }

    // 3. GET http://localhost:3002/authors/{idBlog}
    @GetMapping("/{idAutore}")
    public Authors findById(@PathVariable UUID idAutore) {
        return this.authorsService.findById(idAutore);
    }

    // 4. PUT http://localhost:3002/authors/{idAutore}
    @PutMapping("/{idAutore}")
